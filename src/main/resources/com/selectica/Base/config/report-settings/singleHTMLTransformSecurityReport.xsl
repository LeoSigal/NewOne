...
<xsl:variable name="num-rows" select="number(ReportResults/ReportResultsParameters/ColumnDetails/@size)"/>
	...
	<xsl:template match="ReportResultsOptions">
		...
		<xsl:if test="$incSummaryCount = 'true' or $incSummaryTotals = 'true'  or $incSummaryAvgs = 'true'">
			...
			<div class="Sticky2">
			<fieldset>
			<table id="StatTable" cellpadding="0" cellspacing="0">
				<thead>
...
<!--				<xsl:for-each select="../ColumnDetails/RowGroup">
					...
					<xsl:for-each select="../ColumnHeadings/ColumnHeader">
						<xsl:if test="@reporttotal != '' and $incSummaryTotals = 'true'">
							...
							<xsl:if test="@type = 'Int'">
								<td align="right"><b><xsl:value-of select="format-number(@reporttotal,'#,##0')"/><br/></b></td>
							</xsl:if>
							<xsl:if test="@type = 'Double'">
								<td align="right"><b><xsl:value-of select="format-number(@reporttotal,'#,##0.00')"/><br/></b></td>
							</xsl:if>						
						</xsl:if>
						<xsl:if test="@reportaverage != '' and $incSummaryAvgs = 'true'">
							...
							<xsl:if test="@type = 'Int'">
								<td align="right"><b><xsl:value-of select="format-number(@reportaverage,'#,##0.00')"/><br/></b></td>
							</xsl:if>
							<xsl:if test="@type = 'Double'">
								<td align="right"><b><xsl:value-of select="format-number(@reportaverage,'#,##0.00')"/><br/></b></td>
							</xsl:if>						
	...
	<xsl:template match="RowGroupTotal" mode="summaryTableHeader">
		...
		<xsl:if test="../../../ColumnHeadings/ColumnHeader[$pos]/@ignore != 'true'">
			<xsl:if test="../../../ColumnHeadings/ColumnHeader[$pos]/@type = 'USD'">
				<xsl:attribute name="align">right</xsl:attribute>
			</xsl:if>
	...
	<xsl:template name="formatTotal">
		...
		<xsl:choose>
			...
			<xsl:when test="../../../ColumnHeadings/ColumnHeader[$pos]/@type = 'USD'">
				<xsl:value-of select="format-number($value,'#,##0.00')"/>
			</xsl:when>
			<!-- Int Format (1,000) -->
			<xsl:when test="../../../ColumnHeadings/ColumnHeader[$pos]/@type = 'Int' and (contains(../../../ColumnHeadings/ColumnHeader[$pos]/@name, 'rackingNum'))">
				<xsl:value-of select="format-number($value,'###0')"/>
			</xsl:when>				
			<!-- Int Format (1,000) -->
			<xsl:when test="../../../ColumnHeadings/ColumnHeader[$pos]/@type = 'Int' and not(contains(../../../ColumnHeadings/ColumnHeader[$pos]/@name, 'rackingNum'))">
				<xsl:value-of select="format-number($value,'#,##0')"/>
			</xsl:when>
			<xsl:when test="../../../ColumnHeadings/ColumnHeader[$pos]/@type = 'Double'">
				<xsl:value-of select="format-number($value,'#,##0.00')"/>
			</xsl:when>					
			<!-- Text Format () should not get here! -->
			<xsl:otherwise>
	...
	<xsl:template match="ColumnRow">
		...
		<!-- td>
		<a>
		<xsl:attribute name="href">JavaScript:scheduleLink('<xsl:value-of select="ColumnDetail[$cols]"/>');</xsl:attribute><img src="/pix/meta/view_sm.gif" width="17" height="17" border="0"/></a>
		</td -->
		...
		<td>
			...
			<xsl:choose>
				...
				<xsl:when test="string-length(.) > $max-size">
					...
					<xsl:attribute name="onclick">JavaScript:showExpandedValuePopup('true','<xsl:value-of select="concat('expandedValuePopup_',translate(normalize-space(../../../../ColumnHeadings/ColumnHeader[$pos]),' ',''))"/>',event,this,'<xsl:value-of select="../../../../ColumnHeadings/ColumnHeader[$pos]"/>','<xsl:value-of select="."/>');</xsl:attribute>(more)</a>
				</xsl:when>
				<!-- Text Format () -->
				<xsl:otherwise>
					<xsl:value-of select="."/>
